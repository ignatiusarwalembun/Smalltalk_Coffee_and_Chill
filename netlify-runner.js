...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/health') {
    return json(res, 200, { ok: true, running, lastResult: lastResult.status });
  }
  ...
  if (url.pathname === '/deploy') {
    ...
    if (!proxyPath || !proxyPath.startsWith('https://netlify-mcp.netlify.app/proxy/')) {
      return json(res, 400, { ok: false, message: 'Invalid proxy path' });
    }
